...
type RawLocalQuote = Readonly<{
  text: string;
  author: string | null;
}>;

const rawLocalQuotes = Object.freeze(
[
  {
...
) satisfies readonly RawLocalQuote[];

let memoizedLocalQuotes: readonly Quote[] | null = null;

export function getLocalQuotes(): readonly Quote[] {
  if (!memoizedLocalQuotes) {
    memoizedLocalQuotes = Object.freeze(
      rawLocalQuotes.map((quote) => ({
